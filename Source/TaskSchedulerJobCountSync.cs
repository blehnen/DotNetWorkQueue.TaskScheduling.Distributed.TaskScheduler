...
namespace DotNetWorkQueue.TaskScheduling.Distributed.TaskScheduler
{
    ...
    public class TaskSchedulerJobCountSync: ITaskSchedulerJobCountSync
    {
        ...
        public long GetCurrentTaskCount()
        {
            ...
            _log.CurrentTaskCount(total, myCount, otherCount);
            return total;
        }
        ...
        public void Start()
        {
            ...
            {
                _log.ProcessCommandsFailed(error);
            }
        }
        ...
        private void RunPoller()
        {
            ...
            {
                if (!_disposing)
                    _log.PollerThreadTerminated(error);
            }
        }
        ...
        private void OnActorReady(object sender, NetMQActorEventArgs e)
        {
            try
            {
                ...
                if (message == TaskSchedulerBusCommands.BroadCast.ToString())
                {
                    ...
                    _log.NodeBroadcasting(fromHostAddress);
                ...
                else if (message == TaskSchedulerBusCommands.RemovedNode.ToString())
                {
                    ...
                    _log.NodeRemoved(removedAddress, temp);
                ...
                else
                {
                    ...
                    _log.BroadcastResponse(message);
                }
            }
            catch (Exception error)
            {
                _log.HandleCommandsFailed(error);
            }
        }
            ...
            if (!_disposedValue)
            {
                if (disposing)
                {
                    try
                    {
                        ...
                        {
                            _log.PollerThreadForcedDisposal();
                        }
                        _outbound?.Dispose();
                        _actor?.Dispose();
                        _poller?.Dispose();
